...
def producer(queue, start_time_iso, end_time_iso, hasgeo):
 # the input start_time_iso and end_time_iso are UTC time in ISO-8601 format
    ...
    logging.info('Collection Done:{}-{}'.format(start_time_iso, end_time_iso))
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename="logs/whole_pipeline.log",level=logging.INFO,format="%(asctime)s %(message)s")
    ...
    p1.join()
    ## if p1 hasn't been completed, the main process won't continue (the start_time/end time won;t be changed) 
